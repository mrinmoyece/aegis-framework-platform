...
Supported code is the latest commit on `master`. Layer 2 has production-shaped
OIDC/JWT, PostgreSQL RLS, quota and durable audit boundaries, but live IdP rotation
and production deployment remain unproven. It intentionally has no credentialed
model provider, production evidence source, approval mutation endpoint, or effect.
